...
export function updateProgression(input: UpdateProgressionInput): ProgressionInput {
  if (input.results.length === 0) {
    return input.previous;
  }

  const completionRate =
    input.results.reduce((acc, result) => acc + result.completedSets / Math.max(1, result.plannedSets), 0) /
